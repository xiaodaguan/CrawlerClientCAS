            ...
            while (response == null) {
                if (html.getAgent()) {
                    if (html.getChangeProxy() || html.getProxy() == null) {
                        ...
                        get.setConfig(conf);
                        LOGGER.error("proxy[" + ip_port + "] -> " + html.getOrignUrl());
                        html.setProxy(ip_port);
                    }

//                    LOGGER.error("Cookie: " + html.getCookie());
//                    LOGGER.error("User-Agent: " + html.getUa());
//                    LOGGER.error("Referer: " + html.getReferUrl());
...
//                throw new RuntimeException(e);
    ...
    public void getContent(CrawlTask html, UserAttribute userAttr) {
        ...
        try {
            ...
            if (fromURL == null) {
                LOGGER.error("没有抓取到内容，建议暂停采集！请检查网络链接或URL：" + html.getOrignUrl() + "是否正确。");
    ...
    private void pdf(CrawlTask html, byte[] fromURL) {
        ...
        try {
            ...
            fos.write(fromURL);
            LOGGER.error("pdf保存至： " + fileName);
        } catch (IOException e) {
            LOGGER.error("文件无法下载。", e);
        } finally {
            if (fos != null)
                try {
                    fos.close();
    ...
    public String getJsonContent(String url, String charSet) {
        ...
        // LOGGER.error("encoding: gb2312");
        // return content1;
        // }
        // if (StringUtil.regMatcher(content2, "\\(", "\\)") != null){
        // LOGGER.error("encoding: utf8");
        // return content2;
        // }
