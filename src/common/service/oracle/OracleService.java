...
public abstract class OracleService<T> extends AbstractDBService<T> {
	...
	@Override public void updateStatus(final CrawlerStatus cs, final CrawlerTaskStatus cts, final SearchKey key, final int type) {
		if (type == 1) {// init
			...
			final String sql = "insert into crawler_status"
					+ "(media_type, start_time, status, interval, threads, keyword_all, keyword_current, keyword_exist, url_count, ip, crawler_name, task_name, parent_id, insert_time)"
					+ " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			KeyHolder keyHolder = new GeneratedKeyHolder();
			this.jdbcTemplate.update(new PreparedStatementCreator() {
				@Override public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
			...
			/* 更新父任务(爬虫)状态 */
			this.jdbcTemplate.update("update crawler_status set status = ?, keyword_count=?, complete_count=? where id = ?", cs.getStatus(), cs.getStartKeywordSet().size(), cs.getCrawedCount(),
					cs.getId());
	...
	@Override public Proxy getProxy(int siteId) {
		...
		List<Proxy> proxyList = this.jdbcTemplate.query(sql, new RowMapper<Proxy>() {
			...
			@Override public Proxy mapRow(ResultSet rs, int arg1) throws SQLException {
	...
	@Override public Header randomHeaderFromDB() {
		String sql = "select id, cookie ,user_agent, host, accept, accept_language, accept_encoding, connection, cache_control, referer "
				+ "from headers where site_name = 'sogou'  and cookie not like '%-1%' and trunc(sysdate)-trunc(insert_time)<7 and cookie is not null";
		List<Header> headers = this.jdbcTemplate.query(sql, new RowMapper<Header>() {

			@Override public Header mapRow(ResultSet rs, int arg1) throws SQLException {
	...
	@Override public abstract void saveData(T data);

	@Override public List<SearchKey> searchKeys() {
		...
		if (Systemconfig.clientinfo != null) {
			sql = "select category_code, " + col + " from (select A." + col + ", A.category_code, rownum rn from (select distinct " + col + ", category_code " + "from " + table + clause
					+ ") A where rownum <= " + Systemconfig.clientinfo.getDataEnd() + ") where rn >" + Systemconfig.clientinfo.getDataStart();
		} else {
			sql = "select category_code, " + col + " from " + table + clause;
		}
		System.out.println(sql);
		return this.jdbcTemplate.query(sql, new RowMapper<SearchKey>() {
			@Override public SearchKey mapRow(ResultSet rs, int i) throws SQLException {
